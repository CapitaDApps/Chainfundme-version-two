...
export async function createAccount(formData: FormData): Promise<Account> {
  try {
    ...
    const accountData: Account = {
      ...
      role: (formData.get("role") as "admin" | "fieldAgent" | "customerAgent") || "fieldAgent",
      startDate: formData.get("startDate") as string,
    };

    const response = await axios.post<Account>(`${API_BASE_URL}/account/register`, accountData, {
      headers: {
...
// New function for creating accounts from invitations
export async function createInvitedAccount(formData: FormData): Promise<Account> {
  try {
    const image = formData.get("image") as File | null;
    let imageUrl: string | undefined;
    ...
    const accountData: Account = {
      ...
      password: formData.get("password") as string,
      images: imageUrl ? [imageUrl] : [],
      role: "fieldAgent", // Default role for invited users
      startDate: formData.get("startDate") as string,
    };

    const response = await axios.post<Account>(`${API_BASE_URL}/account/signup`, {
      ...accountData,
...
// New function to generate invite links
export const generateInviteLink = async ({ email, role }: { email: string; role: string }) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/account/invite`, 
      { email, role },
      {
        headers: {
          'Content-Type': 'application/json',
...
// New function to validate invitation tokens
export const validateInvitationToken = async (token: string) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/account/invite/validate?token=${token}`);
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      const errorMessage = error.response?.data?.message || 'Invalid invitation token';
      throw new Error(errorMessage);
    }
    throw new Error('An unexpected error occurred');
  }
